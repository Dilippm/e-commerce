...
const viewCart = async (req, res) => {
    try {
      if (req.session.user_id) {
        ...
        const cartDatas = await User.findOne({ _id:userid}).populate('cart.product').exec()
      ...
      if (req.session.user_id) {
        ...
        if (found) {
          ...
          const cartInserted = await User.updateOne(
            { _id: userId },
            { $push: { cart: { product: prodetails._id, quantity: 1, productTotalPrice: prodetails.price } } }
          );
  
          // Update total price in user document
          const cartData = await User.findOne({ _id: userId }).populate('cart.product');
          const totalPrice = cartData.cart.reduce((total, item) => {
  ...
  const deleteCart = async (req, res, next) => {
    try {
      if (req.session.user_id) {
        ...
        const deleted = await User.updateOne({ _id: userId }, { $pull: { cart: { product: productId } } });
  
        // Recalculate total price
        const user = await User.findOne({ _id: userId }).populate('cart.product').exec();
        let cartTotal = 0;
    ...
    try {
      if (req.session.user_id) {
       ...
       // const price1 = req.body.ptotal
        ...
        const inc = await User.updateOne({ _id: req.session.user_id, "cart.product": prodId }, {
        ...
        const inctotal = await User.updateOne({ _id: req.session.user_id, "cart.product": prodId }, {
          $set: { 'cart.$.productTotalPrice': productprice }
  
        })
        
        const cart = await User.findOne({ _id: req.session.user_id }).populate('cart.product').exec()
let cartTotal = 0;
